...
# Допоміжні функції для виводу імен за id
...
if __name__ == "__main__":
    ...
    print("=== select_1: Топ-5 студентів за середнім балом з усіх предметів ===")
    for row in select_1():
        print(f"  {row.last_name} {row.first_name}: {row.avg_grade}")

    print(f"\n=== select_2: Студент із найвищим середнім балом з предмета '{subject_name}' ===")
    row = select_2(SUBJECT_ID)
    if row:
        print(f"  {row.last_name} {row.first_name}: {row.avg_grade}")

    print(f"\n=== select_3: Середній бал у групах з предмета '{subject_name}' ===")
    for row in select_3(SUBJECT_ID):
        print(f"  Група {row.name}: {row.avg_grade}")

    print("\n=== select_4: Середній бал на потоці (всі предмети, всі студенти) ===")
    print(f"  {select_4()}")

    print(f"\n=== select_5: Курси викладача {teacher_name} ===")
    for row in select_5(TEACHER_ID):
        print(f"  {row.name}")

    print(f"\n=== select_6: Список студентів групи '{group_name}' ===")
    for row in select_6(GROUP_ID):
        print(f"  {row.last_name} {row.first_name}")

    print(f"\n=== select_7: Оцінки студентів групи '{group_name}' з предмета '{subject_name}' ===")
    for row in select_7(GROUP_ID, SUBJECT_ID):
        print(f"  {row.last_name} {row.first_name}: {row.grade} ({row.date_received})")

    print(f"\n=== select_8: Середній бал, який ставить викладач {teacher_name} зі своїх предметів ===")
    print(f"  {select_8(TEACHER_ID)}")

    print(f"\n=== select_9: Список курсів, які відвідує студент {student_name} ===")
    for row in select_9(STUDENT_ID):
        print(f"  {row.name}")

    print(f"\n=== select_10: Курси студента {student_name} у викладача {teacher_name} ===")
